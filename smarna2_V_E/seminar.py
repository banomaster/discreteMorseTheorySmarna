...
#GRADIENT E -> T
EdgCand = set(edgeTriangle.keys()).difference(set(EdgFromVer.keys()))
TriFromEdg = dict()
EdgCri = list()
f1 = open("smarna_edg_arrows.txt","w")
#edge, na katere ne kazejo tocke
for e in EdgCand:
...
#edge, na katere kazejo tocke in zato ne morejo kazati na trikotnike
for e in EdgFromVer.keys():
    f1.write(str({e:None}) + "\n")
...
TriCri=set([tuple(t) for t in T]).difference(set(TriFromEdg.keys()))
